...
/**
 * Fetch Meta (Facebook + Instagram) data via GA4.
 * Filters by source containing "facebook" or "instagram".
 */
export async function fetchMetaData(accessToken, { startDate, endDate }) {
  ...
  const metaFilter = {
    orGroup: {
      expressions: [
        ...
        { filter: { fieldName: "sessionSource", stringFilter: { matchType: "CONTAINS", value: "meta", caseSensitive: false } } },
      ],
    },
  };
  ...
  const [
    ...
    // Facebook vs Instagram breakdown
    runReport(analyticsData, {
      dateRanges: [{ startDate, endDate }],
  ...
  const sourceBreakdown = (sourceBreakdownRes.data.rows ?? []).map((r) => ({
  ...
  return { paid, organic, landingPages, sourceBreakdown };
}
