...
export default function AdsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Google Ads not configured");
      ...
      console.warn("[Google Ads]", err.message);
      setNoData(true);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-6">
      {noData && !loading && <MockBanner message="Google Ads — not connected" />}
      ...
      {noData && !loading && (
        <div className="rounded-2xl border border-border bg-blue-sky/30 p-12 text-center">
          <p className="text-[13px] text-gray-muted">No Google Ads data available for this period</p>
        </div>
      )}
